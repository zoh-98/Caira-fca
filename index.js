...
function buildAPI(globalOptions, html, jar) {
	...
	if (oldFBMQTTMatch) {
		...
		logger(`${cra(`[ CONNECT ]`)} Account region is : ${region}`);
	} else {
		...
		if (newFBMQTTMatch) {
			...
			if (legacyFBMQTTMatch) {
				...
				logger(`${cra(`[ CONNECT ]`)} Account region is : ${region}`);
				logger(`${cra(`[ CONNECT ]`)} Legacy  : ${legacyFBMQTTMatch[6]}`);
			} else {
				log.warn(`${cra(`[ ERROR ]`)} Can't Get ID `);
				noMqttData = html;
			}
		}
	}
...
async function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
		...
		if (appState) {
			...
			if (!process.env['FBKEY']) {
				try {
					...
					fs.writeFile('./../.env', `FBKEY=${ans}`, function(err) {
						if (err) {
							logger(`${cra(`[ ERROR ]`)} Env Failed..`);
						}
						else logger(`${cra(`[ CONNECT ]`)} Env Created..`)
					});
				}
				catch (e) {
					console.log(e);
			...
			if (process.env['FBKEY']) {
				try {
					...
					if (appState.includes('[')) {
						logger(`${cra(`[ CONNECT ]`)} Decrypting AppState`);
					} else {
						try {
							appState = JSON.parse(appState);
							var StateCrypt = require('./StateCrypt');
							appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
							logger(`${cra(`[ CONNECT ]`)} AppState successfully decrypted`);
						}
						catch (e) {
							logger(`${cra(`[ ERROR ]`)} Replace AppState`);
						}
					}
				}
				catch (e) {
			...
			try {
				...
				try {
					...
					return logger(`${cra(`[ ERROR ]`)} Replace AppState`)
				}
			}
			try {
	...
	mainPromise
		.then(function() {
			logger(`${cra(`[ CONNECT ]`)} Logged In Successfully`);
